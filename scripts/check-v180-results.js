...
async function checkV180Results() {
  const { data: version } = await supabase
    .from('versions')
    .select('id, version_number, title, status')
    .eq('version_number', '1.8.0')
    .single();

  if (!version) {
    console.log('Version 1.8.0 not found');
    return;
  }

  console.log('\n📊 Version 1.8.0 - ' + version.title);
  ...
  if (!reports || reports.length === 0) {
    console.log('❌ No test reports found for v1.8.0 yet.\n');
    return;
  }
