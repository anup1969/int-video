          ...
          <div className="mt-3 pt-3 border-t border-gray-200">
            ...
            <div className="mt-3 pt-3 border-t border-gray-200">
              ...
              <div className="flex items-center gap-2">
                <input type="file" accept="audio/*" id="customMusicUpload" className="hidden" onChange={async (e) => { const file = e.target.files[0]; if (file) { if (file.size > 10 * 1024 * 1024) { alert("File too large. Max 10MB."); return; } const { createClient } = await import("@supabase/supabase-js"); const supabase = createClient(window.__NEXT_DATA__.props.pageProps.supabaseUrl || "https://xjxzcmgkidfbiregdcer.supabase.co", window.__NEXT_DATA__.props.pageProps.supabaseKey || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhqeHpjbWdraWRmYmlyZWdkY2VyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDcyODA0NjUsImV4cCI6MjA2Mjg1NjQ2NX0.5AhBRcAIjJLXN1gD6c0WyYbFgKqKg9A4TYJnIHpMy7E"); const timestamp = Date.now(); const randomStr = Math.random().toString(36).substring(7); const ext = file.name.split(".").pop(); const fileName = "audio/" + timestamp + "-" + randomStr + "." + ext; const { data, error } = await supabase.storage.from("campaign-files").upload(fileName, file, { contentType: file.type }); if (error) { alert("Upload error: " + error.message); return; } const { data: urlData } = supabase.storage.from("campaign-files").getPublicUrl(fileName); setEditingStep({ ...editingStep, backgroundMusic: { ...editingStep.backgroundMusic, type: "custom", customUrl: urlData.publicUrl } }); } }} />
