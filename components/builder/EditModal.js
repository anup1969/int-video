          ...
          <div className="mt-3 pt-3 border-t border-gray-200">
            ...
            <div className="mt-3 pt-3 border-t border-gray-200">
              ...
              <div className="flex items-center gap-2">
                <input type="file" accept="audio/*" id="customMusicUpload" className="hidden" onChange={async (e) => { const file = e.target.files[0]; if (file) { const formData = new FormData(); formData.append("file", file); formData.append("type", "audio"); try { const response = await fetch("/api/upload/file", { method: "POST", body: formData }); if (!response.ok) { const text = await response.text(); alert("Upload error: " + text); return; } const result = await response.json(); if (!response.ok) { alert("Upload error: " + (result.error || result.details || "Unknown error")); return; } if (result.fileUrl) { setEditingStep({ ...editingStep, backgroundMusic: { ...editingStep.backgroundMusic, type: "custom", customUrl: result.fileUrl } }); } } catch (err) { alert("Failed to upload music: " + err.message); } } }} />
